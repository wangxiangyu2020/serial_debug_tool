...
│   │   ├── ScriptEditorDialog.h           # JavaScript脚本编辑对话框 (新增)
│   │   └── SettingsTab.h
│   └── utils/             # 工具类头文件 (8个文件)
...
- **样式系统**: 21个专用QSS样式表文件
  ...
  - TCP网络样式：tcp_network_config_tab.qss, tcp_network_client_widget.qss, tcp_network_server_widget.qss (新增)
...
- **工具类**: 6个 (StyleLoader, ThreadPoolManager, SerialPortSettings, JavaScriptHighlighter, NetworkModeState等)
- **图标资源**: 22个SVG + 1个ICO (共23个)
- **样式文件**: 21个QSS文件 (完整的样式系统)
- **Web资源**: 2个文件 (ECharts库 + HTML页面)
- **部署脚本**: 1个 (deploy.bat Windows部署脚本)
- **总文件数**: 约180个文件 (包含构建输出、图片资源和文档)
- **代码行数**: 约30000行 (估算，包含注释和空行)
- **技术栈复杂度**: C++20 + Qt6 + JavaScript + HTML/CSS + WebEngine + 多线程 + 网络通信 + 数据包处理
