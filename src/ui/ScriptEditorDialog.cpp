...
void ScriptEditorDialog::setDefaultScript()
{
    if (m_pScriptEditor)
    {
        m_pScriptEditor->setPlainText(
            ...
            " * 在缓冲区中查找一个完整数据帧的结束位置。\n"
            " * 对于固定长度的帧，此函数非常高效。\n"
            " * @param {Array<number>} buffer - 输入的数据缓冲区(0xEB 0x01 0x00 0xC8 0xED)。\n"
            " * @returns {number} - 如果找到，返回帧在 buffer 中的结束索引；否则返回 -1。\n"
            " */\n"
            "function findFrame(buffer) {\n"
            "    // 示例：查找 0xEB ... 0xED 格式的5字节固定长度帧\n"
            ...
            "    }\n"
            "\n"
            "    // 第2步：根据通道ID，进行\"分拣\"和差异化处理\n"
            "    // 这里的 channelId 需要与添加通道时的通道标识符一致\n"
            "    // 如果你单纯只是想显示文本，那么下面的chartData可以省略，相反。\n"
            "    // 如果你只需要显示图表，那么displayText可以省略\n"
            "    switch (channelId) {\n"
            "        case 1: // 通道 1, 温度数据\n"
            "            return {\n"
            "                displayText: \"channel1: \" + value,\n"
            "                chartData: { channelId: \"ch1\", point: value } \n"
            "            };\n"
            "\n"
            "        case 2: // 通道 2, 湿度数据\n"
            "            return {\n"
            ...
            "\n"
            "/**\n"
            " * (核心函数) 批处理整个数据缓冲区，找出所有完整的数据帧并解析它们。\n"
            " * C++只会调用此函数。此函数内部应调用 findFrame 和 parseFrame。\n"
...
void ScriptEditorDialog::createComponents()
{
    ...
    m_pDescriptionLabel = new QLabel(
        ...
        "3. processBuffer(buffer) - 批处理整个数据缓冲区，找出所有完整的数据帧并解析它们",
        this);
